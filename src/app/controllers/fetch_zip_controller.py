    ...
    async def stream_zip_file(self, zip_path: str):
        ...
        try:
            ...
            if not os.path.isfile(zip_path):
                await self.error_repository.insert_error(Error(f"ZIP file not found: {zip_path}"))
                raise HTTPException(status_code=404, detail=f"ZIP file not found: {zip_path}")
            
            if not zip_path.endswith('.zip'):
                await self.error_repository.insert_error(Error(f"Invalid file format. Expected ZIP file: {zip_path}"))
                raise HTTPException(status_code=400, detail=f"Invalid file format. Expected ZIP file: {zip_path}")
