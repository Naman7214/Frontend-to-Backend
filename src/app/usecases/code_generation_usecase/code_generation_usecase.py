        ...
        try:
            ...
            try:
                ...
                if not endpoints:
                    ...
                    await self._log_error(error_msg)
                    raise HTTPException(status_code=400, detail=f"No endpoints found in file: {input_path}")
            ...
            try:
                ...
                if not auth_template:
                    ...
                    await self._log_error(error_msg)
                    raise HTTPException(status_code=500, detail=f"Failed to load auth template: {auth_filename}")
                
                if not db_template:
                    error_msg = f"Error in CodeGenerationUseCase.execute: Failed to load database template: {db_filename}"
                    await self._log_error(error_msg)
                    raise HTTPException(status_code=500, detail=f"Failed to load database template: {db_filename}")
            except Exception as e:
                error_msg = f"Error in CodeGenerationUseCase.execute: Failed to load reference templates. Error: {str(e)}"
                await self._log_error(error_msg)
                raise HTTPException(status_code=500, detail=f"Failed to load reference templates: {str(e)}")
            ...
            try:
                ...
                if not files:
                    ...
                    await self._log_error(error_msg)
                    raise HTTPException(status_code=500, detail="Failed to parse response - no files generated")
            except Exception as e:
            ...
            try:
                ...
                if not os.path.exists(output_path):
                    ...
                    await self._log_error(error_msg)
                    raise HTTPException(status_code=500, detail=f"Failed to save output to {output_path}")
            except Exception as e:
                error_msg = f"Error in CodeGenerationUseCase.execute: Failed to save output. Error: {str(e)}"
                await self._log_error(error_msg)
                raise HTTPException(status_code=500, detail=f"Failed to save output: {str(e)}")
