...
class CodeGenerationHelper:
    ...
    def load_endpoints(filepath: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise ValueError(f"Expected JSON object in file {filepath}, got {type(data)}")
                
            endpoints = data.get("endpoints", [])
            if not isinstance(endpoints, list):
                raise ValueError(f"Expected 'endpoints' to be a list in {filepath}, got {type(endpoints)}")
            ...
            # Can't log to MongoDB in a static method - handled by caller
            raise ValueError(f"Invalid JSON format in file {filepath}: {str(e)}")
        except Exception as e:
            # Can't log to MongoDB in a static method - handled by caller
            stack_trace = traceback.format_exc()
            raise ValueError(f"Error loading endpoints from {filepath}: {str(e)}\n{stack_trace}")
    ...
    def load_reference_template(filename: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not isinstance(template_data, list):
                raise ValueError(f"Expected JSON array in template file {path}, got {type(template_data)}")
            ...
            # Can't log to MongoDB in a static method - handled by caller
            raise ValueError(f"Invalid JSON format in template file {path}: {str(e)}")
        except Exception as e:
            # Can't log to MongoDB in a static method - handled by caller
            stack_trace = traceback.format_exc()
            raise ValueError(f"Error loading reference template {path}: {str(e)}\n{stack_trace}")

    @staticmethod
    def extract_summary(endpoints: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            if not isinstance(endpoints, list):
                raise ValueError(f"Expected list of endpoints, got {type(endpoints)}")
            ...
            # Can't log to MongoDB in a static method - handled by caller
            stack_trace = traceback.format_exc()
            raise ValueError(f"Error extracting endpoint summaries: {str(e)}\n{stack_trace}")
    ...
    def format_reference_code(template_files: List[Dict[str, Any]]) -> str:
        ...
        try:
            if not isinstance(template_files, list):
                raise ValueError(f"Expected list of template files, got {type(template_files)}")
            ...
            # Can't log to MongoDB in a static method - handled by caller
            stack_trace = traceback.format_exc()
            raise ValueError(f"Error formatting reference code: {str(e)}\n{stack_trace}")
        ...
        try:
            ...
            for i, file in enumerate(result):
                if not isinstance(file, dict):
                    raise ValueError(f"Expected file at index {i} to be a dictionary, got {type(file)}")
                    
                if "file_path" not in file:
                    raise ValueError(f"Missing 'file_path' in file at index {i}")
    ...
    @staticmethod
    async def save_json_output(files: List[Dict[str, Any]], output_path: str) -> None:
        """
        ...
        try:
            ...
            # Can't log to MongoDB in a static method - handled by caller
            stack_trace = traceback.format_exc()
            raise ValueError(f"Error saving JSON output to {output_path}: {str(e)}\n{stack_trace}")
