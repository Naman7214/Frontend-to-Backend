...
class CloneUseCase:
    ...
    async def execute(self, github_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            project_dir = await self.clone_helper.create_project_directory(project_uuid)
            ...
            error_message = f"Validation error in CloneUseCase.execute: {str(val_ex)}"
            error = Error(error_message)
            await self.error_repo.insert_error(error)
            ...
            error_message = f"File system error in CloneUseCase.execute: {str(os_ex)}"
            error = Error(error_message)
            await self.error_repo.insert_error(error)
