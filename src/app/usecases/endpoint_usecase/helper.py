...
class EndpointHelper:
    def __init__(self, openai_service: OpenAIService = Depends(OpenAIService)) -> None:
    ...
    async def find_react_files(self, root_dir, all_files=False, api_files=False, react_hooks=False, auth_files=False):
        ...
        if all_files:
            ...
            patterns.extend([
                ...
                os.path.join(root_dir, "**", "*.js"),  # Include regular JS files too
                os.path.join(root_dir, "**", "*.ts")   # Include TypeScript files
            ])
        elif api_files:
            ...
            patterns.extend([
                ...
                os.path.join(root_dir, "**", "index.js"),  # Include JS index files
    ...
    def update_endpoints_list(self, existing_endpoints, new_endpoints):
        ...
        for new_endpoint in new_endpoints:
            ...
            if new_endpoint.get("isModifiedEndpoint", False) and key in endpoint_map:
                ...
                existing["usedInFiles"] = sorted(list(existing_files.union(new_files)))
                
                # Take the most detailed description
                if len(new_endpoint.get("description", "")) > len(existing.get("description", "")):
                ...
                for param, details in new_endpoint.get("queryParams", {}).items():
    ...
    async def analyze_file_for_endpoints(self, file_path, file_content, root_dir, verbose=False, existing_endpoints=None):
        """
    ...
    async def extract_endpoints(self, root_dir, verbose=False, max_files=None, all_files=False, api_files=False, react_hooks=False, auth_files=False):
        """
        ...
        if verbose:
            print(f"Found {len(react_files)} files (from {', '.join(file_type_desc)})")
        ...
        if verbose:
            ...
            for endpoint in all_endpoints:
                ...
                print(f"  Database required: {endpoint.get('databaseRequired', False)}")
        ...
        if len(all_endpoints) == 0 and verbose:
            ...
            print("  - Using react_hooks=True to find files using data fetching hooks")
            print("  - Using auth_files=True to find authentication-related files")
            print("  - Check if the codebase uses a non-standard API calling pattern")
            print("  - Examine the codebase manually to confirm if it has any API calls")
