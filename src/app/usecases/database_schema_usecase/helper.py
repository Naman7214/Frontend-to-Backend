...
class DatabaseSchemaHelper:
    ...
    async def analyze_endpoint_schema(
        ...
        async with self.semaphore:  # Limit concurrent API calls
            ...
            try:
                ...
                print(
                    f"Error in analyze_endpoint_schema for endpoint {endpoint_data.get('endpointName', 'Unknown')}: {str(e)}"
                )
                return {
    ...
    async def generate_mock_data(
        ...
        async with self.semaphore:  # Limit concurrent API calls
            try:
                ...
                print(
                    f"Generated mock records for collection {schema_info.get('collection_name', 'unknown')}. Target: {num_records} (actual count may vary based on API)."
                )
                return mock_data

            except Exception as e:
                print(
                    f"Error generating mock data for {schema_info.get('collection_name', 'unknown')}: {str(e)}"
                )
    ...
    def insert_to_mongodb(
        ...
        if self.db_repo is None:
            print(
                "DatabaseRepository instance not available. Skipping database insertion."
            )
            return False

        if not data:
            print(
                f"No data provided to insert into collection {collection_name}"
            )
            return False

        if not collection_name or not isinstance(collection_name, str):
            print(
                f"Invalid collection name: {collection_name}. Skipping insertion."
            )
            return False
        ...
        try:
            ...
            if db is None:
                print(
                    f"Failed to initialize database using repo path '{repo_path}'. Skipping insertion."
                )
                return False

            if collection_name in db.list_collection_names():
                print(
                    f"Collection '{collection_name}' in database '{db.name}' already exists. Dropping existing collection."
                )
            ...
            print(
                f"Successfully inserted {len(inserted_ids)} documents into collection '{collection_name}' in database '{db.name}'"
            )
            return True
        except Exception as e:
            print(
                f"Error inserting data into MongoDB collection '{collection_name}' in database for repo '{repo_path}': {str(e)}"
            )
            return False
    ...
    async def insert_to_mongodb_async(
        ...
        if (
            ...
            print(
                f"Invalid parameters for MongoDB insertion: collection={collection_name}, data_length={len(data) if data else 0}"
            )
            return False
        ...
        try:
            ...
            print(
                f"Async error inserting data into MongoDB for {collection_name}: {str(e)}"
            )
            return False
    ...
    async def _process_single_endpoint(
        ...
        if schema and not schema.get("error"):
            ...
            if mock_data:
                ...
                await self.insert_to_mongodb_async(
